...
const CoinTracker = () => {
  ...
  const calcCoinValue = (coins) => {
    ...
    if (numberOfCoins >= platinumDenomination) {
      //able to convert to PP
      coinString =
        coinString + Math.floor(numberOfCoins / platinumDenomination) + " PP ";
      numberOfCoins = numberOfCoins % platinumDenomination;
    }

    if (numberOfCoins >= goldDenomination) {
      //able to convert to GP
      coinString =
        coinString + Math.floor(numberOfCoins / goldDenomination) + " GP ";
      numberOfCoins = numberOfCoins % goldDenomination;
    }

    if (numberOfCoins >= silverDenomination) {
      //able to convert to SP
      coinString =
        coinString + Math.floor(numberOfCoins / silverDenomination) + " SP ";
      numberOfCoins = numberOfCoins % silverDenomination;
    }

    if (numberOfCoins >= 0) {
      //able to convert to CP
  ...
  return (
    <>
      {!hasSubmittedCoins ? (
          ...
          <div className="coin-setters">
            <div className="coin-setter">
              <input
                type="number"
                onChange={(e) =>
                  setCoins(coins + e.target.value * platinumDenomination)
                }
              />
            ...
            <div className="coin-setter">
              <input
                type="number"
                onChange={(e) =>
                  setCoins(coins + e.target.value * goldDenomination)
                }
              />
            ...
            <div className="coin-setter">
              <input
                type="number"
                onChange={(e) =>
                  setCoins(coins + e.target.value * silverDenomination)
                }
              />
            ...
            <div className="coin-setter">
              <input
                type="number"
                onChange={(e) =>
                  setCoins(coins + e.target.value * copperDenomination)
                }
              />
        ...
        <>
          <h2 hidden={!hasSubmittedCoins}>{calcCoinValue(coins)}</h2>
          <div className="manipulate-coins">
            <div className="coin-selector">
              ...
              <Dropdown
                options={[
                  {
                    label: "PP",
                    value: platinumDenomination,
                    type: "platinum-coin",
                  },
                  {
                    label: "GP",
                    value: goldDenomination,
                    type: "gold-coin",
                  },
                  {
                    label: "SP",
                    value: silverDenomination,
                    type: "silver-coin",
                  },
                  {
                    label: "CP",
                    value: copperDenomination,
                    type: "copper-coin",
                  },
                ]}
                value={coinType}
            ...
            <div className="add-subtract">
              <Button
                text="Add"
                textColor="black"
                onClick={() => incrementCoins(numCoins * coinType)}
              />
              <Button
                text="Subtract"
                textColor="black"

...
function App() {
  ...
  const updateConfig = (object, type, num) => {
    setConfigData((currentConfig) => {
      if (currentConfig[type][num] == null) {
        currentConfig[type].push(object);
        console.log(currentConfig[type]);
      } else {
        currentConfig[type][num] = object;
      }
  ...
  const addTrackerSegmentAndClose = (trackerType, data) => {
    setTrackerSegments([
      ...trackerSegments,
      { trackerType: trackerType, data: data },
    ]);
  ...
  const loadConfig = () => {
    ...
    if (json.currentHealth && json.maxHealth) {
      addTrackerSegmentAndClose("Health", {
  ...
  return (
    <div className="App">
      ...
      <div className="tracker-grid">
        {trackerSegments.map((item, i) => (
          <TrackerSegment
            title={item.trackerType + " Tracker"}
            trackerType={item.trackerType.toLowerCase()}
            data={item.data}
            trackerId={i}
        ...
        <div className="add-tracker">
          <Button
            text="+"
            shape="circle"
            onClick={() => newTrackerModal.current.showModal()}
          />
        </div>
      </div>
      ...
      <dialog className="add-tracker-prompt" ref={newTrackerModal}>
        ...
        <Button
          text="Close"
          onClick={() => newTrackerModal.current.close()}
        ></Button>
      </dialog>
      ...
      <dialog className="config-modal" ref={configModal}>
        <div className="config-container">
          ...
          <div className="config-options">
            ...
            <Button text="Print" onClick={() => console.log(configData)} />
          </div>
